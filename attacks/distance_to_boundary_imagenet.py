...
import numpy as np
from utils import average_over_positive_values, wigthed_average, load_Data_with_imagenet_id
import os
...
def distance_to_boundary_imagenet(dataset, num_classes, num_targeted_classes, num_of_samples_per_class, model_name, distance_save_dir, imagenet_path):
    ...
    for j in range(num_targeted_classes):
        ...
        if show_correct_distance:
            ...
            for i in range(cor_class_yes_x.shape[0]):
                distance_per_sample_train[i] = dist_to_boundary(cor_class_yes_x[i:i+1], model, j, norm=2)
            ...
            for i in range(cor_class_no_x.shape[0]):
                distance_per_sample_test[i] = dist_to_boundary(cor_class_no_x[i:i+1], model, j, norm=2)
                if i % reload_session_period == 0:
                    model = reload_session(model_name)
        ...
        if show_incorrect_distance:
            ...
            if incor_class_yes_x.shape[0] < 10 or incor_class_no_x.shape[0] < 10:
                ...
                for i in range(incor_class_yes_x.shape[0]):
                    distance_per_sample_train[i] = dist_to_boundary(incor_class_yes_x[i:i+1], model, j, norm=2)
                    if i % reload_session_period == 0:
                        model = reload_session(model_name)
                        print('Train samples progress: ', i, '/', incor_class_yes_x.shape[0])
                for i in range(incor_class_no_x.shape[0]):
                    distance_per_sample_test[i] = dist_to_boundary(incor_class_no_x[i:i+1], model, j, norm=2)
                    if i % reload_session_period == 0:
                        model = reload_session(model_name)
